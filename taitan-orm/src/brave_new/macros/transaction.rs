...
#[doc(hidden)]
#[macro_export]
macro_rules! new_transaction_impl {
    () => {
        async fn execute<'a>(
            &'a mut self,
            stmt: &'a str,
            args: <Self::DB as sqlx::Database>::Arguments<'a>,
        ) -> taitan_orm_trait::brave_new::result::Result<u64> {
            let ex = &mut *self.transaction;
            Self::generic_execute(ex, stmt, args).await
        }

        async fn execute_plain<'a>(&'a mut self, stmt: &'a str) -> taitan_orm_trait::brave_new::result::Result<u64> {
        ...
        async fn fetch_exists<'a>(
            ...
            stmt: &'a str,
            args: <Self::DB as sqlx::Database>::Arguments<'a>,
        ) -> taitan_orm_trait::brave_new::result::Result<bool> {
            let ex = &mut *self.transaction;
            Self::generic_exists(ex, stmt, args).await
        }

        async fn fetch_exists_plain<'a, A>(&'a mut self, stmt: &'a str) -> taitan_orm_trait::brave_new::result::Result<bool> {
            let ex = &mut *self.transaction;
            let args: std::marker::PhantomData<<Self::DB as sqlx::Database>::Arguments<'a>> =
                std::marker::PhantomData::default();
        ...
        async fn fetch_option<'a, SE>(
            ...
            selection: &'a SE,
            args: <Self::DB as sqlx::Database>::Arguments<'a>,
        ) -> taitan_orm_trait::brave_new::result::Result<Option<SE>>
        where
            SE:  taitan_orm_trait::brave_new::Selected<Self::DB> + Send + Unpin,
        {
            let ex = &mut *self.transaction;
        ...
        async fn fetch_all<'a, SE>(
            ...
            selection: &'a SE,
            args: <Self::DB as sqlx::Database>::Arguments<'a>,
        ) -> taitan_orm_trait::brave_new::result::Result<Vec<SE>>
        where
            SE:  taitan_orm_trait::brave_new::Selected<Self::DB> + Send + Unpin,
        {
            let ex = &mut *self.transaction;
        ...
        async fn fetch_one_full<'a, SE>(
            ...
            stmt: &'a str,
            args: <Self::DB as sqlx::Database>::Arguments<'a>,
        ) -> taitan_orm_trait::brave_new::result::Result<SE>
        where
            SE:  taitan_orm_trait::brave_new::Selected<Self::DB> + Send + Unpin,
        {
            let ex = &mut *self.transaction;
        ...
        async fn fetch_one_full_plain<'a, SE>(&'a mut self, stmt: &'a str) -> taitan_orm_trait::brave_new::result::Result<SE>
        where
            SE:  taitan_orm_trait::brave_new::Selected<Self::DB> + Send + Unpin,
        {
            let ex = &mut *self.transaction;
