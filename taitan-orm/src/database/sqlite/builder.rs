...
use std::fs;
use std::path::Path;
use taitan_orm_trait::error::DatabaseInitFail;
...
impl SqliteBuilder {
    async fn init_local(workspace_dir: &str, db_file: &str) -> crate::result::Result<SqlitePool> {
        ...
        let workspace_absolute = workspace.absolutize().map_err(|_e| {
            TaitanOrmError::DatabaseInitFail(DatabaseInitFail("workdir absolute fail".to_string()))
        })?;

        fs::create_dir_all(&workspace_absolute).map_err(|_e| {
            TaitanOrmError::DatabaseInitFail(DatabaseInitFail("create dir fail".to_string()))
        })?;
        ...
        let sqlite_pool = SqlitePool::connect_with(options).await.map_err(|_e| {
