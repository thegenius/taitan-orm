...
use taitan_orm_parser::{ConditionsMapper, Connector, MySqlKeywordEscaper, TableDef};
...
fn condition_mapper_spec() {
    ...
    let conditions_mapper = ConditionsMapper::default();
    let escaper = MySqlKeywordEscaper::default();

    let sets = conditions_mapper
        .connect_dynamic(&table_def.fields, &escaper)
        .to_string();
    ...
    let sets = conditions_mapper
        .connect_dynamic_indexed(&table_def.fields, &escaper)
        .to_string();
    assert_eq!(sets, "{ let mut s = String :: default () ; let mut has_prev = false ; let mut index = 1 ; s . push_str (format ! (\"a{}${}\" , self . a . get_cmp_sql () , index) . as_ref ()) ; s . push_str (format ! (\",b{}${}\" , self . b . get_cmp_sql () , index) . as_ref ()) ; s . push_str (format ! (\",c{}${}\" , self . c . get_cmp_sql () , index) . as_ref ()) ; has_prev = true ; if self . d . is_some () { s . push_str (format ! (\",d{}${}\" , self . d . get_cmp_sql () , index) . as_ref ()) ; index = index + 1 ; } if self . e . is_some () { s . push_str (format ! (\",e{}${}\" , self . e . get_cmp_sql () , index) . as_ref ()) ; index = index + 1 ; } s . push_str (format ! (\"f{}${}\" , self . f . get_cmp_sql () , index) . as_ref ()) ; s . push_str (format ! (\",g{}${}\" , self . g . get_cmp_sql () , index) . as_ref ()) ; if self . h . is_some () { s . push_str (format ! (\",h{}${}\" , self . h . get_cmp_sql () , index) . as_ref ()) ; index = index + 1 ; } s . push_str (format ! (\"i{}${}\" , self . i . get_cmp_sql () , index) . as_ref ()) ; ; s }");
}
