...
impl TemplateArgTraitImplGenerator {
    pub fn gen_add_to_args(
        ...
        quote! {
            impl taitan_orm_trait::brave_new::TemplateArgTrait<sqlx::#db_ident> for #struct_ident {
                fn add_to_args<'a, 'b>(&'a self, name: &'b str, args: &'b mut <sqlx::#db_ident as sqlx::Database>::Arguments<'a>) -> taitan_orm_trait::brave_new::result::Result<()> {
                    #stream
                    Ok(())
                }
            }
