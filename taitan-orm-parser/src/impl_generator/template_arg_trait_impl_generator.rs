...
impl TemplateArgTraitImplGenerator {
    pub fn gen_add_to_args(
        ...
        quote! {
            impl taitan_orm::traits::TemplateArgTrait<sqlx::#db_ident> for #struct_ident {
                fn add_to_args<'a, 'b>(&'a self, name: &'b str, args: &'b mut <sqlx::#db_ident as sqlx::Database>::Arguments<'a>) -> taitan_orm::result::Result<()> {
                    #stream
                    Ok(())
                }
            }
